...
- In WebXR: resets to a fixed pose (`(0, 1.3, -1.1)`, target rotation) the
  ...
  and smoothly slerps `xrRotationRef` towards the target focus quaternion of the `selectedChannel` (or default resting posture) — unless
...
- On the 2D desktop viewport: scales the model to occupy ~1/3 of the
  ...
  bobbing motion; otherwise it smoothly auto-rotates (slerps) the headset to focus on the `selectedChannel`'s LED facing directly forward.
