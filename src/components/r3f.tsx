  ...
  return (
    <div className="w-full h-full flex flex-col md:flex-row relative bg-white overflow-hidden text-slate-800 font-sans">
      ...
      <div className="flex-1 flex flex-col relative bg-transparent">
        ...
        {/* Top-Left Playback Controls (Play/Pause & Speed) */}
        {!isIdle && (
          <div className="absolute top-4 right-4 z-30 flex items-center gap-2 bg-slate-900/90 border border-slate-700/50 backdrop-blur-md rounded-full p-1.5 shadow-xl">
            {/* Play / Pause Toggle Button */}
            <button
              onClick={engine.togglePlayPause}
              className="flex items-center justify-center p-2 rounded-full bg-slate-800 hover:bg-slate-700 text-white transition-all active:scale-95 cursor-pointer relative group"
              title={engine.isPaused ? "Play (Spacebar)" : "Pause (Spacebar)"}
            >
              {engine.isPaused ? (
                <svg className={`w-5 h-5 fill-current transition-colors duration-300 ${engine.frame.phase === "baseline" ? "text-indigo-400" : "text-emerald-400"}`} viewBox="0 0 24 24">
            ...
            {/* Speed Toggle (1x / 10x) */}
            <div className="flex items-center bg-slate-950/80 p-0.5 rounded-full border border-slate-800 select-none font-mono text-sm font-bold">
              <button
                onClick={() => engine.setSpeed(1)}
                className={`px-2 py-0.5 rounded-full transition-all cursor-pointer ${engine.speed === 1
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded-full transition-all cursor-pointer ${engine.speed === 10
                  ? `${engine.frame.phase === "baseline" ? "bg-indigo-600" : "bg-emerald-600"} text-white shadow-sm font-extrabold`
                  : "text-slate-400 hover:text-slate-200"
                  }`}
              >
        ...
        {/* TOP CENTER PHASE HUD */}
        {engine.mode.kind === "demo" && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 pointer-events-none select-none">
            <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-700/50 backdrop-blur-md px-4 py-2 rounded-full shadow-2xl transition-all duration-300">
              <span className={`w-2.5 h-2.5 rounded-full ${engine.frame.phase === "baseline"
                ? "bg-indigo-500 shadow-[0_0_10px_#6366f1]"
                : "bg-emerald-500 shadow-[0_0_10px_#10b981]"
                } animate-pulse`} />
              <span className={`text-[10px] md:text-xs font-black tracking-[0.2em] uppercase transition-all duration-300 ${engine.frame.phase === "baseline" ? "text-indigo-400" : "text-emerald-400"
                }`}>
                {engine.frame.phase === "baseline" ? "Baseline Phase" : "Stimulation Phase"}
              </span>
            </div>
          </div>
        )}
